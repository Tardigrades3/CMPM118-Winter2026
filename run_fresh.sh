...
#   Stage 3  — git commit + push  (arch JSON + DIL CL JSONs)
#   Stage 4  — CL-method HPO     (CIL scenario, 3 methods)
#   Stage 5  — Full CIL sweep    (all subjects × all modes × both archs)
#   Stage 6  — git commit + push  (CIL CL JSONs + eval results from this run)
#
# Usage
# ─────
...
#   --skip-git-mid    skip Stage 3 (intermediate git commit)
#   --skip-cil-hpo    skip Stage 4
#   --skip-cil-sweep  skip Stage 5
#   --skip-git-final  skip Stage 6
...
_git_add_if_exists() {
  # Stage a file only if it exists (avoids git error on missing optional files)
  local f="$1"
  if [[ -f "$f" ]]; then
    echo "  git add $f"
    $DRY_RUN || git add "$f"
...
# ── Stage 3: Git commit — arch + DIL HPO results ─────────────────────────────
_banner "Stage 3 — Git commit: arch + DIL HPO results"
if $SKIP_GIT_MID; then
  ...
  _git_commit_push "hpo: arch and DIL CL method results (${ARCH})"
fi

# ── Stage 4: CL-method HPO — CIL ─────────────────────────────────────────────
_banner "Stage 4 — CL Method HPO (CIL scenario)"
echo "  subjects : $HPO_SUBJ_CL"
echo "  trials   : $N_TRIALS_CL each"
...
# ── Stage 5: Full CIL sweep ───────────────────────────────────────────────────
_banner "Stage 5 — Full CIL sweep"
...
if $SKIP_CIL_SWEEP; then
  ...
  # Mark timestamp so Stage 6 only commits eval files from this run
...
# ── Stage 6: Git commit — CIL HPO + eval results ─────────────────────────────
_banner "Stage 6 — Git commit: CIL HPO results + eval JSONs"
if $SKIP_GIT_FINAL; then
  ...
  # Only add eval files created during this run (avoids re-staging old results)
  if [[ -f /tmp/run_fresh_cil_sweep_start ]]; then
    echo "  Staging new eval results..."
